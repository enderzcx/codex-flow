...
CI 会在 push 到 `main` 和 pull request 时跑同一组非 live smoke。它会检查本地 workflow registry、验证 `diff-review`，并确认带写能力但没有 gate 的 workflow 会失败；默认不会在 CI 里启动 live Codex worker。
